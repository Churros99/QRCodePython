...
class FormMestroDesign(tk.Tk):
    ...
    def __init__(self):
        ...
        self.qr_image_base = util_img.leer_imagen("./imagenes/qr_base.png", (330, 330))
    ...
    def paneles(self):
        ...
        self.barra_superior_izq = tk.Frame(self.barra_superior, bg=conf.COLOR_BARRA_SUPERIOR_IZQ)
        self.barra_superior_izq.pack(side=tk.LEFT, fill='both', expand=True)
        
        self.barra_superior_der = tk.Frame(self.barra_superior, bg=conf.COLOR_BARRA_SUPERIOR_DER)
        self.barra_superior_der.pack(side=tk.RIGHT, fill='both', expand=True)

        self.barra_inferior = tk.Frame(self, bg=conf.COLOR_BARRA_INFERIOR, height=50)
        self.barra_inferior.pack(side=tk.BOTTOM, fill='x', expand=False)
    
    def widget_panel_superior(self):
        self.labelTitulo = tk.Label(self.barra_superior_izq, text="QR Diseño de Código", bg=conf.COLOR_BARRA_SUPERIOR_IZQ)
        self.labelTitulo.config(fg="#000000", font=("Roboto", 18, "bold"), pady=10, width=16)
        self.labelTitulo.pack(side=tk.TOP, expand=False, pady=20)
        
        estilo = ttk.Style()
        estilo.configure("EstiloEntry.TEntry", padding=(10, 5, 10, 5), relief="flat")
        
        self.entrada_texto = ttk.Entry(self.barra_superior_izq, width=50, style="EstiloEntry.TEntry")
        ...
        self.etiqueta_qr = ttk.Label(self.barra_superior_der,image= self.qr_image_base)
        self.etiqueta_qr.pack(side=tk.TOP, expand=True, pady=20, padx=20)
        
    def widget_panel_inferior(self):
        self.boton_generar = tk.Button(
            self.barra_inferior, text="Generar Código", width=15, height=2, bg=conf.COLOR_BOTON, fg=conf.COLOR_TEXTO_BOTON, relief=tk.FLAT, padx=5, pady=5, 
        ...
        self.boton_descargar = tk.Button(
            self.barra_inferior, text="Descargar Código", width=15, height=2, bg=conf.COLOR_BOTON, fg=conf.COLOR_TEXTO_BOTON, relief=tk.FLAT, padx=5, pady=5, 
    ...
    def on_button_click_mostrar_qr(self, event):
        pass
    
    def on_button_click_guardar_qr(self, event):
        pass